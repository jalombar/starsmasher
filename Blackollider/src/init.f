...
c     compute look-up tables:
     ...
     $     nav,nintvar,neos,ncooling
      ...
      if(myrank.eq.0) write(69,*)'Reading in aa and bb values...'
      open(99,file='sph.passivelyAdvected')
      read(99,*, end=125) (aa(i),bb(i),dd(i),i=1,nmax)
 125  close(99)
...
c         endif
     ...
     $              ueq(i),tthermal(i)
         ...
         if(nintvar.eq.12) then
            if(myrank.eq.0) then
               write(69,*) 'Switching from A to u'
            endif
            if(neos.eq.0) then
               do i=1,ntot
                  u(i)=u(i)*rho(i)**(gam-1)/(gam-1d0)
                  udot(i)=0d0
               enddo
            else if (neos.eq.1) then
               if(myrank.eq.0)
     $              write(69,*)' Will use meanmolecularweight=',
     $              meanmolecularweight 
               do i=1,ntot
                  meanmolecular(i)=meanmolecularweight
                  pcgs=u(i)*rho(i)**gam*punit
                  rhocgs=rho(i)*munit/runit**3
                  call gettemperature(2*qconst*rhocgs/meanmolecular(i),
     $                 -3*pcgs/arad,temperature)
                  ucgs=arad*(qconst*temperature/meanmolecular(i)+temperature**4/rhocgs)
                  u(i)=ucgs/(gravconst*munit/runit)
                  udot(i)=0d0
               enddo
            else
               write(69,*)'Currently can switch from nintvar=1 to 2 '
               write(69,*)'only if neos=0 or 1'
               stop
            endif
            nintvar=2
         endif
 ...
 101     format (' init: parameters for this ideal + radiation run:',/,
     ...
     $        ' corepts=',i2,/)
         ...
         elseif(nintvar.eq.2)then
            ...
            if(nintvar.eq.12) write(69,*) 'nintvar=12 is for ',
     $           'switching from nintvar=1 to 2 when a ',
     $           'restartrad.sph file is being used.'
            stop
         endif
         if(neos.eq.0) then
         ...
         write(69,*)'usegravitycorrections=',usegravitycorrections
         if(.not. usegravitycorrections) then
            write(69,*)'gravity of all particles to be softened with h=',
     $           hco
         endif
...
c     set some default values, so that they don't necessarily have to be set in the sph.input file:
      ...
      hceiling=1d30               ! hp(i) = hptilde(i) + hfloor, where hp(i)=smoothing length and hptilde(i) is used in eq.(A1) of GLPZ 2010.
      AACONST=1d30             ! parameter in analytic relation between h and rho: equals the maximum allowed smooting length
      BBCONST=1d0              ! parameter in analytic relation between h and rho
