...
c     derived constants:
     ...
     $     integratednum,maxmu,minmu,numlines
      ...
      if(gflag.eq.0)then
         hc=0.5d0*radius*(1d0*nnopt/n)**(1.d0/3.d0) ! neighbor number is about 1.8*nnopt, roughly
      else
...
c     below here, n=total number of particles
     ...
     $           nnopt/anumden)**(1.d0/3.d0) !   + hfloor
            ...
            aa(i)=hceiling-hfloor
            dd(i)=hfloor
! Originally (without dd)
!            bb(i)=(32d0*pi/3/
!     $           am(i)/NNOPT)**(1d0/3d0) - 1/aa(i)/rhoex**(1d0/3d0)
!     Could have been           
!            bb(i)=(1/hp(i) - 1/aa(i))/rhoex**(1d0/3d0)
...
c      do i=1,n
c         call newnene(i)
c         call bonetsum(i,rho(i),drhodhi)
c         aa(i)=AACONST
c         bb(i)=BBCONST
c         do icount=1,9
c            hp(i)=1.d0/(1.d0/aa(i)+bb(i)*rho(i)**(1.d0/3.d0))
c            call bonetsum(i,rho(i),drhodhi)
c            print *,i,icount,hp(i),rho(i),nn(i)
c         enddo
c      enddo
...
c     profilefile comes from MESA
 ...
 99      continue
     ...
     $              trim(headers(i)).eq.'x' .or.
     $              trim(headers(i)).eq.'h1') then
     ...
     $              trim(headers(i)).eq.'z') then
            ...
            elseif(trim(headers(i)).eq.'radius') then
               iradius=i
            elseif(trim(headers(i)).eq.'pressure') then
               ipressure=i
         ...
         if(myrank.eq.0) then
            ...
            write(69,*) imass,': mass'
            if(ilogR.ne.0) write(69,*) ilogR,': log_10 radius'
            if(iradius.ne.0) write(69,*) iradius,': radius'
            write(69,*) ilogT,': log_10 temperature'
            write(69,*) ilogRho,': log_10 density'
            if(ilogP.ne.0) write(69,*) ilogP,' : log_10 pressure'
            if(ipressure.ne.0) write(69,*) ipressure,' : pressure'
            write(69,*) ix_mass_fraction_H,' : X'
            if(iy_mass_fraction_He.ne.0)write(69,*)iy_mass_fraction_He,
     $           ' : Y'
            if(ihe3.ne.0)write(69,*)ihe3, ' : He3 abundance'
            if(ihe4.ne.0)write(69,*)ihe4, ' : He4 abundance'

            write(69,*) iz_mass_fraction_metals,' : Z'

            if(imass.eq.0) then
               write(69,*)'Could not find mass data'
               stop
            endif
            if(ilogR.eq.0 .and. iradius.eq.0) then
               write(69,*)'Could not find logR or radius data'
               stop
            endif
            if(ilogT.eq.0) then
               write(69,*)'Could not find logT data'
               stop
            endif
            if(ilogRho.eq.0) then
               write(69,*)'Could not find logRho data'
               stop
            endif
            if(ilogP.eq.0 .and. ipressure.eq.0) then
               write(69,*)'Could not find logP or pressure data'
               stop
            endif
            if(ix_mass_fraction_H.eq.0) then
               write(69,*)'Could not find X data'
               stop
            endif
            if(iy_mass_fraction_He.eq.0) then
               write(69,*)'Could not find Y data: will use Y=He3+He4'
            endif
            if(iz_mass_fraction_metals.eq.0) then
               write(69,*)'Could not find Z data: will use Z=1-X-Y'
            endif
         endif
...
c            read(io,*) zone, (mesadata(col), col=2,numcol)
            read(io,*) (mesadata(col), col=1,numcol)
            xm(i)= mesadata(imass)
            if(ilogR.gt.0) then
               logR=mesadata(ilogR)
            else if(iradius.gt.0) then
               logR=log10(mesadata(iradius))
            else
               if(myrank.eq.0)write(69,*)'Problem with radius'
               stop
            endif
            logT=mesadata(ilogT)
            ...
            if(ilogP.gt.0) then
               logP=mesadata(ilogP)
            elseif(ipressure.gt.0) then
               logP=log10(mesadata(ipressure))
            else
               if(myrank.eq.0)write(69,*)'Problem with pressure'
               stop
            endif   
            ...
            else if(ihe3.ne.0 .and. ihe4.ne.0) then
               y_mass_fraction_He=mesadata(ihe3)+mesadata(ihe4)
            else if (ihe4.ne.0) then
               y_mass_fraction_He=mesadata(ihe4)
            else if (ihe3.ne.0) then
               y_mass_fraction_He=mesadata(ihe3)
            else
               if(myrank.eq.0)write(69,*)'Problem with helium abundance'
               stop
            endif
     ...
     $              z_mass_fraction_metals - 1d0
               write(69,*)' X =', x_mass_fraction_H
               write(69,*)' Y =', y_mass_fraction_He
               write(69,*)' Z =', z_mass_fraction_metals
               stop
            endif
...
c            write(69,'(9g13.5)')xm(i),rarray(i),pres(i),rhoarray(i),
c     $           muarray(i),x_mass_fraction_H,y_mass_fraction_He,
c     $           z_mass_fraction_metals
            maxmu=max(maxmu,muarray(i))    
            minmu=min(minmu,muarray(i))    
            if(abs(x_mass_fraction_H + y_mass_fraction_He +      
     ...
     $              z_mass_fraction_metals -1d0,'mass problem in zone',k          
            ...
            i=i-1               ! Because we are reading from the outside in
         enddo           
         close(io)       
         numlines=num_zones                
         xm(numlines+1)=xm(numlines)       
