...
c     (a3 vector)=(8/3)^0.5*a1*(z hat)
      ...
      ixmax=int(rpreedge/a1)+2
      iymax=int(rpreedge/(3.d0**0.5d0/2.d0*a1))+2
      izmax=int(rpreedge/(0.5d0*(8.d0/3.d0)**0.5d0*a1))+2
      rnearby=0d0
      do ix=-ixmax,ixmax
         do iy=-iymax,iymax
            do iz=1,izmax
     ...
     $              -(mod(abs(iz),2)-0.5d0)*1.d0/3.d0**0.5d0*a1
               ...
               call sph_splint(rprearray,rarray,rarray2,numlines,
     $              rtry,rpost)
               if(rpost.lt.redge) then
...
c     (particle is accepted)    
                  ...
                  if(rtry.le.a1) then
                     rnearby=max(rnearby,(x(ip)**2+y(ip)**2+z(ip)**2)**0.5d0)
                     if(myrank.eq.0) then
...
c     below here, n=total number of particles
      ...
      if(corepts.gt.0) then
         ...
         if(mco.gt.0.d0) then
            am(1)=mco
c     At this point, amtot=mass of SPH particles and am(1) is the mass of the core
c     This is not guaranteed to be the desired mass amass.  Most likly, amtot is a little
c     too small. We want the SPH particle mass to increase by amass-amtot-am(1) overall.
c     That will make the new SPH mass amass-am(1). Therefore, for all SPH particles,
c     increase their mass by the factor (amass-am(1))/amtot
            count=0
            do i=1+corepts,ntot
c               am(i)=am(i)*(amass-am(1))/amtot
               rtry=(x(i)**2+y(i)**2+z(i)**2)**0.5d0
               if(rtry.le.rnearby) then
                  am(i)=am(i)+(amass-amtot-am(1))/6
                  count=count+1
                  if(myrank.eq.0) write(69,*)'particle',i,
     $                 'will be given more mass'
               endif
            enddo
            if(count.ne.6) then
               write(69,*) 'rnearby=',rnearby
               write(69,*) 'count=',count
               write(69,*) 'There should be 6 close particles'
               stop
            endif
         else
            am(1)=amass-amtot
         ...
         amtotal=0
         do i=1,ntot
            amtotal=amtotal+am(i)
         enddo
         if(myrank.eq.0) then
            write(69,*) 'mass of core = ',am(1),'msun'
            write(69,*) 'mass of entire star = ',amtotal,'msun'
         endif
         if(hco.gt.0.d0) then
            hp(1)=hco
         else
