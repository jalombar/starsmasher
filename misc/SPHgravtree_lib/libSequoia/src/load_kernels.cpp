...
void octree::allocateInternalBuffers()
{
  ...
  this->atomicValues.setContext(devContext);
  this->atomicValues.cmalloc(10, false);  
  ...
  localTree.activePartList.cmalloc(n_bodies, false);   //Active particles, TODO for later
...
#else
  propsNonLeafD.load_source("./compute_propertiesD.ptx", pathName.c_str(), "");
  propsLeafD.load_source("./compute_propertiesD.ptx", pathName.c_str(), "");
  propsScalingD.load_source("./compute_propertiesD.ptx", pathName.c_str(), "");
#endif
...
//   approxGrav.load_source("./dev_approximate_gravity.cubin", "", "", 64);
  ...
  */
  ...
  getNGB.load_source("./dev_get_ngb.ptx", pathName.c_str(), "", 64);
  //getNGB.load_source("./dev_get_grp_ngb.ptx", pathName.c_str(), "", 64);
  /* create kernels */
/*
