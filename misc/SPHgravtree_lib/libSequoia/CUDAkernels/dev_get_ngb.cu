...
__device__ __forceinline__ int ngb_count(float4 pos_i,
                         float h_i,
                         float3 pos_j)
{
...
__device__ uint get_n_ngb(int DIM2x, int DIM2y,
  ...
  /*********** shared memory distribution **********/
                                                  ...
                                                  //  begin,    end,   size
                                                  // -----------------------
  int *direct = (int*)&shmem[ 0];                 //  0*DIM,  1*DIM,  1*DIM     //Stores unprocessed particles (dont overwrite)
  int *nodes  = (int*)&direct[   DIM];            //  1*DIM, 11*DIM, 10*DIM     //max extract: 8*64 +64 = 9*DIM use 10 for now
                                                                                //Keep 1xDIM in between passes!
  int *prefix = (int*)&nodes [10*DIM];            //  11*DIM, 13*DIM,  2*DIM    //Used at same time as nodes and body list, let not overlap

  int    *body_list = (int*   )&nodes    [  DIM]; //   2*DIM,  6*DIM,  4*DIM     //Used to extract particles
  float3 *sh_pos    = (float3*)&body_list[4*DIM]; //   6*DIM,  9*DIM,  3*DIM
  int    *sh_jid    = (int*  )&sh_pos[DIM];       //   9*DIM, 10*DIM,  1*DIM    
  //int *sh_body     = &sh_jid[DIM];              //  10*DIM ,11*DIM,  1*DIM     //created in the loop itself
...
//         split = true;
	...
	***/
        ...
        /*** if DIM shared memory or more is filled with the the nodes, dump these into slowmem stack ***/
	while(n_offset >= DIM) {
	  n_offset -= DIM;
	...
	int nl_pre = 0;                                                     //Number of leaves that have already been processed
        ...
        #define NJMAX (DIM*4)
	while (n_bodies > 0) {
	  int nb    = min(n_bodies, NJMAX - n_direct);                    //Make sure number of bodies to be extracted does not exceed
...
// 	      if (body_i != sh_jid[offs + j]) //If statement replaced by multiplication
              {
                direCount++;
#ifdef DONGBCOUNT
                n_ngb += selfGrav*ngb_count(pos_i, h_group, sh_pos[offs + j]);
#endif
              }
	    }//End for j < DIMx
	    __syncthreads();
...
dev_get_n_ngb(const int n_active_groups,
  ...
  const int blockDim2 = NTHREAD2;
   __shared__ int shmem[13*(1 << blockDim2)];
...
//Uniform cube distribution with [0,1]^3 search range is :
//~(Nngb/N)^(1/3)
//(100 / 1.000.000)^(1/3) = 0.046415
    h_group = 0.046715;
    h_group = 0.0315;
    ...
    n_ngb = get_n_ngb<blockDim2, 0>( DIM2x, DIM2y, tid, tx, ty,
                                            ...
                                            shmem, lmem, direCount, boxSizeInfo,
                                            curGroupSize, boxCenterInfo, 
                                            h_group, bodies_ids);
