...
```text
parallel_bleeding_edge/bin/starsmasher_cpu
```
...
For that reason, the GPU build is recommended for normal Apple Silicon runs, and the examples below use `starsmasher_gpu`.
...
```bash
mpirun -np 2 /path/to/starsmasher/parallel_bleeding_edge/bin/starsmasher_gpu
```
