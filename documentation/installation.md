...
The first thing is that you must have one or more NVIDIA Graphic Cards. Why? 

There are 2 version of the code. The GPU version and the CPU version. The GPU version of the code uses Nvidia graphic cards to calculate gravity and let the CPU do the rest (and less) of the work. The CPU version of the code has the CPU handle all the calculations.

Depending on your machine, in most cases (unless you have a very high number of CPU per node) the GPU version of the code is much faster than the CPU version of the code. For example, an NVIDIA GeForce GTX 950M can handle 150,000 Particles with the GPU version of the code in the same amount of time that the CPU version handles 15,000 particles on a dual quad-core intel i7).	

Then you must need an NVIDIA graphics card to run StarSmasher, not others.  
If you have more than one NVIDIA graphics card, they should be of the same compute capability version: this wikipedia [page](https://en.wikipedia.org/wiki/CUDA) gives the compute capability version of each NVIDIA graphic card. If, for example, your machine has an Nvidia TITAN Xp, a Titan X and a GeForce GTX 1080 Ti, then they will calculate gravity correctly because they are all a 6.1 compute capability version. You can’t have, however, a Titan X and a NVIDIA TITAN V, because one is a 6.1, the other is a 7.0. This means that they will calculate gravity both in a different way and StarSmasher will fail to run properly. So please be sure that all of your graphic cards are of the same compute capability version if not the same.
...
The 3rd thing that we suggest is to use Linux as an OS to run StarSmasher. This doesn’t mean that you won’t be able to install StarSmasher in another Linux OS, MacOS, or Windows.  Just know that you can met difficulties and waste a lot of time if you are not skilled and experienced on installing libraries.
